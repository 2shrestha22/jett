...
async fn send_one(
    ...
    let mut handle = tokio::fs::File::open(&file.source).await?;
    if resume_from > 0 {
        handle.seek(std::io::SeekFrom::Start(resume_from)).await?;
    }
    ...
    match outcome {
        ...
        Ok(response) => {
            ...
            let message = format!("peer refused {}: {status} {detail}", file.source.display());
